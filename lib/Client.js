            ...
            this.on("message", function (e) {
                ...
                switch (res.t) {
                    ...
                    case "MESSAGE_CREATE":
                        ...
                        var next_1 = function (req, res, arr, i, secoundArr) {
                            if (i === void 0) { i = 0; }
                            return function () {
                                arr[i + 1] ? arr[i + 1].cb(req, res, next_1(req, res, arr, i++)) : (secoundArr ? (secoundArr[0] ? secoundArr[0].cb(req, res, next_1(req, res, secoundArr, i++)) : 0) : 0);
                            };
                        };
                        var _1 = [];
                        self.middleware.forEach(function (v) { return _1.push({ cb: v }); });
                        self.middleware[0] ? self.middleware[0](request, response, next_1(request, response, _1, 0, command)) : 0;
                        command ? command[0].cb(request, response, next_1(request, response, command)) : 0;
                }
            });
        });
    };
